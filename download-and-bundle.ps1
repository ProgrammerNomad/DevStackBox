...
function Expand-DownloadedArchive {
    ...
    try {
        ...
        # For some archives, content might be in a subdirectory - move it up
        $subDirs = Get-ChildItem -Path $extractPath -Directory
        if ($subDirs.Count -eq 1) {
            $subDir = $subDirs[0]
            $tempPath = "$extractPath\_temp"
            Move-Item $subDir.FullName $tempPath
            Remove-Item $extractPath -Recurse -Force
            Move-Item $tempPath $extractPath
        }
...
foreach ($component in $downloads.GetEnumerator()) {
    ...
    if (-not (Test-Path $downloadPath) -or $Force) {
        if (-not (Get-DownloadedFile $config.url $downloadPath $name)) {
            continue
        }
    } else {
    ...
    # Extract
    if (Expand-DownloadedArchive $downloadPath $extractPath $name) {
...
Write-Host "📊 Successfully processed: $downloadedComponents/$totalComponents components" -ForegroundColor Cyan
Write-Host ""
...
if ($finalComponents -eq $totalComponents) {
    ...
    Write-Host ""
    Write-Host "▶️  Run 'npm start' to launch DevStackBox!" -ForegroundColor Cyan
} else {
    Write-Host "⚠️  Some components are still missing ($finalComponents/$totalComponents)" -ForegroundColor Yellow
